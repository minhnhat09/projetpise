...
//--- MODIFIER OEUVRE ---//
if(!empty($_POST))
{	//pour éviter une erreur si aucune img n'est ajoutée
	$img_bdd = ""; 
	if(isset($_GET['action']) && $_GET['action'] == 'modifier')
	{
		...
		{
			// Inspection de la taille du fichier, ici moins de 10Ko
			if ($_FILES["img"]["size"] <= 10000)
			{
				...
				echo "<br/>La taille du fichier est trop importante (10 Ko max) !";
			}
		}
		/*else {
			echo "<br/>Fichier inexistant ou en erreur !";
		}*/
...
$contenu .= "<br/><a href=\"?action=affichage\">Affichage des oeuvres</a><br/>";
$contenu .= "<a href=\"?action=ajouter\">Ajout d'une oeuvre</a><br/><br/><hr /><br/>";
//--- AFFICHAGE PRODUITS ---//
if(isset($_GET['action']) && $_GET['action'] == "affichage")
{
	...
	//$contenu .= 'Nombre d\'oeuvre(s) dans la boutique : ' . count($ligne);
...
if(isset($_GET['action']) && ($_GET['action'] == 'ajouter' || $_GET['action'] == 'modifier'))
{
	...
	{
        $produit_actuel = ListeParID("oeuvre","ID_Oeuvre",$_GET['ID_Oeuvre']);
	}
	echo '
	<h1> Formulaire Produits </h1>
	<form method="post" enctype="multipart/form-data" action="">
	
		<input type="hidden" id="ID_Oeuvre" name="ID_Oeuvre" value="'; if(isset($produit_actuel['ID_Oeuvre'])) echo $produit_actuel['ID_Oeuvre']; echo '" />
			
		<label for="Nom_Oeuvre">Nom_Oeuvre</label><br/>
		<input type="text" id="Nom_Oeuvre" name="Nom_Oeuvre" value="'; if(isset($produit_actuel['Nom_Oeuvre'])) echo $produit_actuel['Nom_Oeuvre']; echo '" /><br/><br/>

		<label for="Cat_Oeuvre">Cat_Oeuvre</label><br/>
		<select name="Cat_Oeuvre">
		<option value="Peinture"'; if(isset($produit_actuel) && $produit_actuel['Cat_Oeuvre'] == 'Peinture') echo ' selected '; echo '>Peinture</option>
		<option value="Sculpture"'; if(isset($produit_actuel) && $produit_actuel['Cat_Oeuvre'] == 'Sculpture') echo ' selected '; echo '>Sculpture</option>
		<option value="Photographie"'; if(isset($produit_actuel) && $produit_actuel['Cat_Oeuvre'] == 'Photographie') echo ' selected '; echo '>Photographie</option>
        </select><br/><br/>
        
		<label for="Taille">Taille</label><br/>
		<input type="text" id="Taille" name="Taille" value="'; if(isset($produit_actuel['Taille'])) echo $produit_actuel['Taille']; echo '"  /> <br/><br/>

		<label for="Prix">Prix</label><br/>
		<textarea name="Prix" id="Prix">'; if(isset($produit_actuel['Prix'])) echo $produit_actuel['Prix']; echo '</textarea><br/><br/>
		
		<label for="Artiste">Artiste</label><br/>
		<input type="text" id="Artiste" name="Artiste" value="'; if(isset($produit_actuel['Artiste'])) echo $produit_actuel['Artiste']; echo '" /> <br/><br/>

		<label for="img">img</label><br/>
		<input type="file" id="img" name="img" /><br/><br/>';
		if(isset($produit_actuel))
		{
			echo '<i>Vous pouvez télécharger une nouvelle img si vous souhaitez la changer</i><br/>';
		...
		<input type="submit" value="'; echo strtoupper($_GET['action']). '"/>
	</form>';
}
