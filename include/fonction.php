...
function Connexion($Pseudo, $Mdp)
{
    ...
    if ($donnees) {
        ...
        if ($donnees['mdp'] == $Mdp) {   //créer une session avec les éléments de la base de données
            foreach ($donnees as $indice => $element) {   //condition pour ne pas pas prendre mdp dans la session
                if ($indice != 'mdp') {
...
function ListeParID($Table, $Col, $Comparer, $ID)  // fonction de r�cup�ration de peinture, scupture, photographie
{
    include "db_params.php";
    ...
    $Requete_SQL = 'SELECT * FROM ' . $Table . " WHERE " . $Col . "" . $Comparer . " '" . $ID . "'";
    $resultat = $bdd->prepare($Requete_SQL);
    $resultat->execute();
    $resultat->setFetchMode(PDO::FETCH_ASSOC);
    return $resultat;
